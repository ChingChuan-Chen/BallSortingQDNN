...
def imbalanced_env():
    ...
    num_balls_per_tube = np.array([2, 3, 4, 4, 2, 1], dtype=np.int8)
    return C_BallSortEnv(tube_capacity=4, num_colors=4, num_empty_tubes=2, state=state, num_balls_per_tube=num_balls_per_tube)
...
def test_top_index(env, imbalanced_env):
    ...
    assert imbalanced_env.top_index(0) == 1
    ...
    assert imbalanced_env.top_index(5) == 0
...
def test_get_top_color_streak(env, imbalanced_env):
    ...
    assert imbalanced_env.get_top_color_streak(0) == 2
    ...
    assert imbalanced_env.get_top_color_streak(5) == 1
...
def test_is_valid_move(env, imbalanced_env):
    ...
    assert imbalanced_env.is_valid_move(5, 1)  # Can move to matched color
    assert not imbalanced_env.is_valid_move(0, 1)  # Cannot move to non-matched color
...
def test_get_valid_actions(env, imbalanced_env):
    expected_actions = [(0, 4), (0, 5), (1, 4), (1, 5), (2, 4), (2, 5), (3, 4), (3, 5)]
    assert sorted(env.get_valid_actions()) == sorted(expected_actions)
    assert sorted(imbalanced_env.get_valid_actions()) == sorted([(0, 4), (1, 5), (4, 0), (5, 1)])
...
def test_num_balls_per_tube(env, imbalanced_env):
    ...
    assert np.array_equal(imbalanced_env.get_num_balls_per_tube(), np.array([2, 3, 4, 4, 2, 1], dtype=np.int8))
